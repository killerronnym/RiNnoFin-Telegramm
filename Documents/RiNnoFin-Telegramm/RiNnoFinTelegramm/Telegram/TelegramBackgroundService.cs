...
public sealed class TelegramBackgroundService : IHostedService, IDisposable
{
    ...
    private const int InactivityThresholdHours = 168; // 1 Woche - vorher 24h triggerte taeglich
    ...
    private void CheckForInactivity(object? state)
    {
        ...
        _logger.LogDebug(
            "Telegram-Bot war für {Hours:F1} Stunden inaktiv. Reorganisation auslösen...",
            inactivityDuration.TotalHours);

        ConfigureBot(_plugin.Configuration);

        // BUG-FIX: LastActivityTime zuruecksetzen damit nicht jede 30 Min erneut
        // getriggert wird. Ohne Fix blieb LastActivityTime auf altem Wert (600+ Std).
        if (_botService != null)
        {
            _botService.LastActivityTime = DateTime.UtcNow;
        }
    }
