    ...
    def install_update(self, zipball_url, new_version, published_at):
        def _run():
            try:
                ...
                # 2.5 Backup vor dem Überschreiben erstellen (Rollback-Sicherheit)
                rollback_dir = os.path.join(self.project_root, "data", "rollback_backup")
                if os.path.exists(rollback_dir):
                    shutil.rmtree(rollback_dir)
                
                log.info("Erstelle Rollback-Backup vor dem Update...")
                # Wir sichern nur die Dateien, die wir potenziell überschreiben würden
                os.makedirs(rollback_dir, exist_ok=True)
                # Hier kopieren wir einfach den gesamten App-Ordner (ohne data/instance/venv)
                for item in os.listdir(self.project_root):
                    s = os.path.join(self.project_root, item)
                    d = os.path.join(rollback_dir, item)
                    # Überspringe Ordner, die wir ohnehin nicht anfassen
                    if item in ['.git', 'venv', '.venv', 'data', 'instance', 'logs', 'shared_bot_utils.py', 'shared_utils.py', 'shared_models.py', 'app.db']:
                        continue
                    try:
                        if os.path.isdir(s):
                ...
                rollback_dir = os.path.join(self.project_root, "data", "rollback_backup")
                if os.path.exists(rollback_dir):
                    log.info("Update fehlgeschlagen. Starte automatisches Rollback...")
                    try:
                        for item in os.listdir(rollback_dir):
                            s = os.path.join(rollback_dir, item)
                        ...
                        log.error(f"Rollback ebenfalls fehlgeschlagen: {rb_err}")
