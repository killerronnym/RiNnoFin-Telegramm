        ...
        with flask_app.app_context():
            ...
            if success and os.path.exists(gif_path):
                with open(gif_path, 'rb') as animation:
                    sent_msg = await bot.send_animation(animation=animation, **kwargs)
                os.remove(gif_path)
            else:
                sent_msg = await bot.send_message(chat_id=chat_id, text=message_text, parse_mode='HTML', message_thread_id=topic_id)
                
            if sent_msg and AutoCleanupTask and db and settings.get('auto_delete_wishes', True):
                days = int(settings.get('auto_delete_wishes_days', 7))
                hours = int(settings.get('auto_delete_wishes_hours', 0))
                minutes = int(settings.get('auto_delete_wishes_minutes', 0))
                
                # Fallback if everything is set to 0
                if days == 0 and hours == 0 and minutes == 0:
                    days = 7
                    
                cleanup_at = datetime.utcnow() + timedelta(days=days, hours=hours, minutes=minutes)
                task = AutoCleanupTask(
                ...
                db.session.add(task)
                db.session.commit()
                logger.info(f"Scheduled auto-deletion of birthday wish {sent_msg.message_id} in {sent_msg.chat_id} for {cleanup_at}")
